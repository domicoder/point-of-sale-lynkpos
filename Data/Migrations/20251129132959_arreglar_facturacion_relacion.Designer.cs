// <auto-generated />
using System;
using Data;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Metadata;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;

#nullable disable

namespace Data.Migrations
{
    [DbContext(typeof(AppDbContext))]
    [Migration("20251129132959_arreglar_facturacion_relacion")]
    partial class arreglar_facturacion_relacion
    {
        /// <inheritdoc />
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("ProductVersion", "9.0.5")
                .HasAnnotation("Relational:MaxIdentifierLength", 128);

            SqlServerModelBuilderExtensions.UseIdentityColumns(modelBuilder);

            modelBuilder.Entity("Domain.Models.Caja", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier")
                        .HasColumnName("id");

                    b.Property<bool>("Activo")
                        .HasColumnType("bit")
                        .HasColumnName("activo");

                    b.Property<DateTime?>("ActualizadoEn")
                        .HasColumnType("datetime2")
                        .HasColumnName("actualizado_en");

                    b.Property<string>("Codigo")
                        .IsRequired()
                        .HasMaxLength(30)
                        .HasColumnType("nvarchar(30)")
                        .HasColumnName("codigo");

                    b.Property<DateTime>("CreadoEn")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("datetime2")
                        .HasColumnName("creado_en")
                        .HasDefaultValueSql("CURRENT_TIMESTAMP");

                    b.Property<bool>("Eliminado")
                        .HasColumnType("bit")
                        .HasColumnName("eliminado");

                    b.Property<short>("EstadoId")
                        .HasColumnType("smallint")
                        .HasColumnName("estado_id");

                    b.Property<string>("Nombre")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)")
                        .HasColumnName("nombre");

                    b.HasKey("Id")
                        .HasName("pk_cajas");

                    b.HasIndex("Codigo")
                        .IsUnique()
                        .HasDatabaseName("caja_udx_codigo");

                    b.HasIndex("EstadoId")
                        .HasDatabaseName("ix_cajas_estado_id");

                    b.ToTable("cajas", (string)null);
                });

            modelBuilder.Entity("Domain.Models.CajaBitacora", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier")
                        .HasColumnName("id");

                    b.Property<DateTime?>("ActualizadoEn")
                        .HasColumnType("datetime2")
                        .HasColumnName("actualizado_en");

                    b.Property<Guid>("CajaId")
                        .HasColumnType("uniqueidentifier")
                        .HasColumnName("caja_id");

                    b.Property<DateTime>("CreadoEn")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("datetime2")
                        .HasColumnName("creado_en")
                        .HasDefaultValueSql("CURRENT_TIMESTAMP");

                    b.Property<DateTime>("FechaApertura")
                        .HasColumnType("datetime2")
                        .HasColumnName("fecha_apertura");

                    b.Property<DateTime?>("FechaCierre")
                        .HasColumnType("datetime2")
                        .HasColumnName("fecha_cierre");

                    b.Property<Guid>("UsuarioId")
                        .HasColumnType("uniqueidentifier")
                        .HasColumnName("usuario_id");

                    b.HasKey("Id")
                        .HasName("pk_caja_bitacora");

                    b.HasIndex("CajaId")
                        .HasDatabaseName("ix_caja_bitacora_caja_id");

                    b.HasIndex("UsuarioId")
                        .HasDatabaseName("ix_caja_bitacora_usuario_id");

                    b.ToTable("caja_bitacora", (string)null);
                });

            modelBuilder.Entity("Domain.Models.CajaEstadoModel", b =>
                {
                    b.Property<short>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("smallint")
                        .HasColumnName("id");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<short>("Id"));

                    b.Property<DateTime?>("ActualizadoEn")
                        .HasColumnType("datetime2")
                        .HasColumnName("actualizado_en");

                    b.Property<DateTime>("CreadoEn")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("datetime2")
                        .HasColumnName("creado_en")
                        .HasDefaultValueSql("CURRENT_TIMESTAMP");

                    b.Property<string>("Nombre")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)")
                        .HasColumnName("nombre");

                    b.HasKey("Id")
                        .HasName("pk_caja_estados");

                    b.ToTable("caja_estados", (string)null);

                    b.HasData(
                        new
                        {
                            Id = (short)1,
                            CreadoEn = new DateTime(1, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Nombre = "CERRADO"
                        },
                        new
                        {
                            Id = (short)2,
                            CreadoEn = new DateTime(1, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Nombre = "ABIERTO"
                        });
                });

            modelBuilder.Entity("Domain.Models.EstadoFacturaModel", b =>
                {
                    b.Property<short>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("smallint")
                        .HasColumnName("id");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<short>("Id"));

                    b.Property<DateTime?>("ActualizadoEn")
                        .HasColumnType("datetime2")
                        .HasColumnName("actualizado_en");

                    b.Property<DateTime>("CreadoEn")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("datetime2")
                        .HasColumnName("creado_en")
                        .HasDefaultValueSql("CURRENT_TIMESTAMP");

                    b.Property<string>("Nombre")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)")
                        .HasColumnName("nombre");

                    b.HasKey("Id")
                        .HasName("pk_estados_facturas");

                    b.ToTable("estados_facturas", (string)null);

                    b.HasData(
                        new
                        {
                            Id = (short)1,
                            CreadoEn = new DateTime(1, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Nombre = "EMITIDA"
                        },
                        new
                        {
                            Id = (short)2,
                            CreadoEn = new DateTime(1, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Nombre = "CANCELADA"
                        });
                });

            modelBuilder.Entity("Domain.Models.Factura", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier")
                        .HasColumnName("id");

                    b.Property<DateTime?>("ActualizadoEn")
                        .HasColumnType("datetime2")
                        .HasColumnName("actualizado_en");

                    b.Property<Guid>("CajaId")
                        .HasColumnType("uniqueidentifier")
                        .HasColumnName("caja_id");

                    b.Property<DateTime>("CreadoEn")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("datetime2")
                        .HasColumnName("creado_en")
                        .HasDefaultValueSql("CURRENT_TIMESTAMP");

                    b.Property<short>("EstadoId")
                        .HasColumnType("smallint")
                        .HasColumnName("estado_id");

                    b.Property<DateTime>("FechaEmision")
                        .HasColumnType("datetime2")
                        .HasColumnName("fecha_emision");

                    b.Property<decimal>("Impuestos")
                        .HasPrecision(12, 2)
                        .HasColumnType("decimal(12,2)")
                        .HasColumnName("impuestos");

                    b.Property<decimal>("Subtotal")
                        .HasPrecision(12, 2)
                        .HasColumnType("decimal(12,2)")
                        .HasColumnName("subtotal");

                    b.Property<short>("TipoId")
                        .HasColumnType("smallint")
                        .HasColumnName("tipo_id");

                    b.Property<decimal>("Total")
                        .HasPrecision(12, 2)
                        .HasColumnType("decimal(12,2)")
                        .HasColumnName("total");

                    b.Property<Guid>("UsuarioId")
                        .HasColumnType("uniqueidentifier")
                        .HasColumnName("usuario_id");

                    b.HasKey("Id")
                        .HasName("pk_facturas");

                    b.HasIndex("CajaId")
                        .HasDatabaseName("ix_facturas_caja_id");

                    b.HasIndex("EstadoId")
                        .HasDatabaseName("ix_facturas_estado_id");

                    b.HasIndex("TipoId")
                        .HasDatabaseName("ix_facturas_tipo_id");

                    b.HasIndex("UsuarioId")
                        .HasDatabaseName("ix_facturas_usuario_id");

                    b.ToTable("facturas", (string)null);
                });

            modelBuilder.Entity("Domain.Models.FacturaDetalle", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier")
                        .HasColumnName("id");

                    b.Property<DateTime?>("ActualizadoEn")
                        .HasColumnType("datetime2")
                        .HasColumnName("actualizado_en");

                    b.Property<int>("Cantidad")
                        .HasColumnType("int")
                        .HasColumnName("cantidad");

                    b.Property<DateTime>("CreadoEn")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("datetime2")
                        .HasColumnName("creado_en")
                        .HasDefaultValueSql("CURRENT_TIMESTAMP");

                    b.Property<Guid>("FacturaId")
                        .HasColumnType("uniqueidentifier")
                        .HasColumnName("factura_id");

                    b.Property<decimal>("ImpuestoPorcentaje")
                        .HasPrecision(4, 2)
                        .HasColumnType("decimal(4,2)")
                        .HasColumnName("impuesto_porcentaje");

                    b.Property<decimal>("Impuestos")
                        .HasPrecision(12, 2)
                        .HasColumnType("decimal(12,2)")
                        .HasColumnName("impuestos");

                    b.Property<string>("NombreProducto")
                        .IsRequired()
                        .HasMaxLength(250)
                        .HasColumnType("nvarchar(250)")
                        .HasColumnName("nombre_producto");

                    b.Property<decimal>("PrecioUnitario")
                        .HasPrecision(12, 2)
                        .HasColumnType("decimal(12,2)")
                        .HasColumnName("precio_unitario");

                    b.Property<string>("ProductoId")
                        .IsRequired()
                        .HasMaxLength(36)
                        .HasColumnType("nvarchar(36)")
                        .HasColumnName("producto_id");

                    b.Property<decimal>("Subtotal")
                        .HasPrecision(12, 2)
                        .HasColumnType("decimal(12,2)")
                        .HasColumnName("subtotal");

                    b.Property<decimal>("Total")
                        .HasPrecision(12, 2)
                        .HasColumnType("decimal(12,2)")
                        .HasColumnName("total");

                    b.HasKey("Id")
                        .HasName("pk_facturas_detalles");

                    b.HasIndex("FacturaId")
                        .HasDatabaseName("ix_facturas_detalles_factura_id");

                    b.ToTable("facturas_detalles", (string)null);
                });

            modelBuilder.Entity("Domain.Models.Rol", b =>
                {
                    b.Property<short>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("smallint")
                        .HasColumnName("id");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<short>("Id"));

                    b.Property<DateTime?>("ActualizadoEn")
                        .HasColumnType("datetime2")
                        .HasColumnName("actualizado_en");

                    b.Property<DateTime>("CreadoEn")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("datetime2")
                        .HasColumnName("creado_en")
                        .HasDefaultValueSql("CURRENT_TIMESTAMP");

                    b.Property<string>("Nombre")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)")
                        .HasColumnName("nombre");

                    b.HasKey("Id")
                        .HasName("pk_roles");

                    b.ToTable("roles", (string)null);

                    b.HasData(
                        new
                        {
                            Id = (short)1,
                            CreadoEn = new DateTime(1, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Nombre = "ADMIN"
                        });
                });

            modelBuilder.Entity("Domain.Models.TipoFacturaModel", b =>
                {
                    b.Property<short>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("smallint")
                        .HasColumnName("id");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<short>("Id"));

                    b.Property<DateTime?>("ActualizadoEn")
                        .HasColumnType("datetime2")
                        .HasColumnName("actualizado_en");

                    b.Property<DateTime>("CreadoEn")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("datetime2")
                        .HasColumnName("creado_en")
                        .HasDefaultValueSql("CURRENT_TIMESTAMP");

                    b.Property<string>("Nombre")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)")
                        .HasColumnName("nombre");

                    b.HasKey("Id")
                        .HasName("pk_tipos_facturas");

                    b.ToTable("tipos_facturas", (string)null);

                    b.HasData(
                        new
                        {
                            Id = (short)1,
                            CreadoEn = new DateTime(1, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Nombre = "DEBITO"
                        },
                        new
                        {
                            Id = (short)2,
                            CreadoEn = new DateTime(1, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Nombre = "CREDITO"
                        });
                });

            modelBuilder.Entity("Domain.Models.Usuario", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier")
                        .HasColumnName("id");

                    b.Property<bool>("Activo")
                        .HasColumnType("bit")
                        .HasColumnName("activo");

                    b.Property<DateTime?>("ActualizadoEn")
                        .HasColumnType("datetime2")
                        .HasColumnName("actualizado_en");

                    b.Property<DateTime>("CreadoEn")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("datetime2")
                        .HasColumnName("creado_en")
                        .HasDefaultValueSql("CURRENT_TIMESTAMP");

                    b.Property<bool>("Eliminado")
                        .HasColumnType("bit")
                        .HasColumnName("eliminado");

                    b.Property<string>("Nombre")
                        .IsRequired()
                        .HasMaxLength(150)
                        .HasColumnType("nvarchar(150)")
                        .HasColumnName("nombre");

                    b.Property<string>("Password")
                        .IsRequired()
                        .HasMaxLength(1000)
                        .HasColumnType("nvarchar(1000)")
                        .HasColumnName("password");

                    b.Property<short>("RolId")
                        .HasColumnType("smallint")
                        .HasColumnName("rol_id");

                    b.Property<string>("UsuarioNombre")
                        .IsRequired()
                        .HasMaxLength(30)
                        .HasColumnType("nvarchar(30)")
                        .HasColumnName("usuario_nombre");

                    b.HasKey("Id")
                        .HasName("pk_usuarios");

                    b.HasIndex("RolId")
                        .HasDatabaseName("ix_usuarios_rol_id");

                    b.HasIndex("UsuarioNombre")
                        .IsUnique()
                        .HasDatabaseName("udx_usuario_nombre")
                        .HasFilter("[Eliminado] = 0");

                    b.HasIndex("Eliminado", "Activo")
                        .HasDatabaseName("idx_usuario_eliminado_activo");

                    b.ToTable("usuarios", (string)null);

                    b.HasData(
                        new
                        {
                            Id = new Guid("bfe03e22-65e4-4007-8420-07c1b53c4726"),
                            Activo = true,
                            CreadoEn = new DateTime(1, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            Eliminado = false,
                            Nombre = "Admin",
                            Password = "9U0zeOGybSi5hk81k/nFzw==.FN5jpe1k2hAMfU0SIg2QuTiwVdhsFdYsC1ykHHAwkzk=",
                            RolId = (short)1,
                            UsuarioNombre = "admin"
                        });
                });

            modelBuilder.Entity("Domain.Models.Caja", b =>
                {
                    b.HasOne("Domain.Models.CajaEstadoModel", "CajaEstado")
                        .WithMany("Cajas")
                        .HasForeignKey("EstadoId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("caja_fk_estado_id");

                    b.Navigation("CajaEstado");
                });

            modelBuilder.Entity("Domain.Models.CajaBitacora", b =>
                {
                    b.HasOne("Domain.Models.Caja", "Caja")
                        .WithMany("CajaBitacoras")
                        .HasForeignKey("CajaId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("caja_bitacora_fk_caja_id");

                    b.HasOne("Domain.Models.Usuario", "Usuario")
                        .WithMany("CajaBitacoras")
                        .HasForeignKey("UsuarioId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("caja_bitacora_fk_usuario_id");

                    b.Navigation("Caja");

                    b.Navigation("Usuario");
                });

            modelBuilder.Entity("Domain.Models.Factura", b =>
                {
                    b.HasOne("Domain.Models.Caja", "Caja")
                        .WithMany("Facturas")
                        .HasForeignKey("CajaId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("facturas_fk_caja_id");

                    b.HasOne("Domain.Models.EstadoFacturaModel", "EstadoFactura")
                        .WithMany("Facturas")
                        .HasForeignKey("EstadoId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("facturas_fk_estado_id");

                    b.HasOne("Domain.Models.TipoFacturaModel", "TipoFactura")
                        .WithMany("Facturas")
                        .HasForeignKey("TipoId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("facturas_fk_tipo_id");

                    b.HasOne("Domain.Models.Usuario", "Usuario")
                        .WithMany("Facturas")
                        .HasForeignKey("UsuarioId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("facturas_fk_usuario_id");

                    b.Navigation("Caja");

                    b.Navigation("EstadoFactura");

                    b.Navigation("TipoFactura");

                    b.Navigation("Usuario");
                });

            modelBuilder.Entity("Domain.Models.FacturaDetalle", b =>
                {
                    b.HasOne("Domain.Models.Factura", "Factura")
                        .WithMany("FacturaDetalle")
                        .HasForeignKey("FacturaId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("facturas_detalles_fk_factura_id");

                    b.Navigation("Factura");
                });

            modelBuilder.Entity("Domain.Models.Usuario", b =>
                {
                    b.HasOne("Domain.Models.Rol", "Rol")
                        .WithMany("Usuarios")
                        .HasForeignKey("RolId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("usuarios_fk_rol_id");

                    b.Navigation("Rol");
                });

            modelBuilder.Entity("Domain.Models.Caja", b =>
                {
                    b.Navigation("CajaBitacoras");

                    b.Navigation("Facturas");
                });

            modelBuilder.Entity("Domain.Models.CajaEstadoModel", b =>
                {
                    b.Navigation("Cajas");
                });

            modelBuilder.Entity("Domain.Models.EstadoFacturaModel", b =>
                {
                    b.Navigation("Facturas");
                });

            modelBuilder.Entity("Domain.Models.Factura", b =>
                {
                    b.Navigation("FacturaDetalle");
                });

            modelBuilder.Entity("Domain.Models.Rol", b =>
                {
                    b.Navigation("Usuarios");
                });

            modelBuilder.Entity("Domain.Models.TipoFacturaModel", b =>
                {
                    b.Navigation("Facturas");
                });

            modelBuilder.Entity("Domain.Models.Usuario", b =>
                {
                    b.Navigation("CajaBitacoras");

                    b.Navigation("Facturas");
                });
#pragma warning restore 612, 618
        }
    }
}
